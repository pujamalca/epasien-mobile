...
<div class="page">
    <div class="topbar">
        ...
        <div class="topbar-title">General Consent</div>
    </div>
        ...
        <div id="signView" style="display:none;">
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:16px;">
                ...
                <label style="display:flex;align-items:center;gap:6px;font-size:var(--font-sm);cursor:pointer;">
                    <input type="checkbox" id="confirmCheck"> Saya menyetujui general consent ini
                </label>
            </div>
            <button class="btn btn-primary" id="btnSign">Setujui &amp; Kirim</button>
        </div>
    </div>
</div>
@endsection
...
(function () {
    ...
    function showSignView(noReg) {
        currentNoReg = noReg;
        document.getElementById('signTitle').textContent = 'No. Registrasi: ' + noReg;
    ...
    document.getElementById('btnSign').addEventListener('click', async function () {
        ...
        try {
            ...
            var data = await apiFetch('/api/consent/sign/general-consent', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ no_reg: currentNoReg, tanda_tangan: ttd, confirmed: true })
            });
            if (data && data.success) {
                showToast('General consent berhasil disimpan', 'success');
                setTimeout(function () { showListView(); loadList(); }, 1200);
            } else {
                showToast((data && data.error) || 'Gagal menyimpan', 'error');
            }
        } catch (e) { showToast(e.message, 'error'); }
        btn.disabled = false; btn.textContent = 'Setujui & Kirim';
    });

    async function loadList() {
        showLoading('container');
        try {
            var data = await apiFetch('/api/consent/list/general-consent');
            var c = document.getElementById('container');
            c.textContent = '';
            if (!data || !data.data || data.data.length === 0) {
                showEmpty('container', '✍️', 'Tidak ada general consent');
                return;
            }
            data.data.forEach(function (d) {
                var row  = el('div', { className: 'card', style: 'margin-bottom:8px;cursor:pointer;' });
                var noReg = el('div', { style: 'font-weight:600;' }, 'No. Reg: ' + d.no_reg);
                var tgl  = el('div', { style: 'font-size:var(--font-sm);color:var(--color-text-muted);' }, d.tgl_setuju ? 'Tanggal: ' + d.tgl_setuju : '');
                var badgeCls = d.status === 'Setuju' ? 'badge badge-success' : 'badge badge-warning';
                var badge = el('span', { className: badgeCls, style: 'margin-top:6px;' }, d.status === 'Setuju' ? 'Sudah Disetujui' : 'Menunggu TTD');
                row.appendChild(noReg);
                row.appendChild(tgl);
                row.appendChild(badge);
                if (d.status !== 'Setuju') {
                    row.addEventListener('click', function () { showSignView(d.no_reg); });
                }
                c.appendChild(row);
            });
        } catch (e) {
    ...
    window._loadGeneralConsent = loadList;
})();
...
@push('init')
<script type="module">window._loadGeneralConsent && window._loadGeneralConsent();</script>
@endpush
